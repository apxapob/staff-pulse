...
const node = (id: string, parentId: string | null, headcount: number, performance: number, budget: number): OrgNode => ({
  id, name: id, parentId, headcount, performance, budget,
  updatedAt: '2026-09-14T12:00:00Z',
});

describe('calculateAggregates', () => {
  it('includes own metrics and all descendants using headcount-weighted performance', () => {
    const aggregates = calculateAggregates(buildOrgIndex([
    ...
    expect(aggregates.get('root')).toEqual({ headcount: 20, budget: 1000, performance: 49, nodeCount: 4 });
    expect(aggregates.get('department')).toEqual({ headcount: 8, budget: 500, performance: 85, nodeCount: 2 });
    expect(aggregates.get('team')).toEqual({ headcount: 5, budget: 300, performance: 100, nodeCount: 1 });
  ...
  it('does not average child averages and excludes zero-headcount performance', () => {
    ...
    expect(aggregates.get('root')).toEqual({ headcount: 10, budget: 250, performance: 82, nodeCount: 4 });
  ...
  it('returns null performance for an empty-employee subtree while still summing budgets', () => {
    const aggregates = calculateAggregates(buildOrgIndex([
      node('root', null, 0, 100, 0.1),
      node('child', 'root', 0, 0, 0.2),
    ]));
  ...
  it('aggregates a deep hierarchy without recursion', () => {
    const nodes = Array.from({ length: 15_000 }, (_, i) => node(String(i), i === 0 ? null : String(i - 1), 1, 60, 2));
    const aggregates = calculateAggregates(buildOrgIndex(nodes));
    expect(aggregates.get('0')).toEqual({ headcount: 15_000, budget: 30_000, performance: 60, nodeCount: 15_000 });
  });
});
